          ...
          <tbody>
            @foreach ($perawatan as $row)
              <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                ...
                <td class="px-6 py-4">{{ $row->perawatan }}</td>
          ...
          <form action="{{ route('jenis-barang.store') }}" method="POST">
            ...
            <div class="space-y-4">
              <div>
                ...
                <input type="text" name="kode_perawatan" id="kode_perawatan" autocomplete="off"
              ...
              <div>
                <label for="perawatan" class="block text-sm font-medium text-gray-900">Jenis Barang</label>
                <input type="text" name="perawatan" id="perawatan" autocomplete="off"
                  class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2" />
                @error('perawatan')
                  <small class="text-red-500 text-sm mt-1"> {{ $message }}</small>
                @enderror
      ...
      <div class="bg-white rounded-lg shadow-xl w-full max-w-md">
        ...
        <div class="p-4">
          <form id="updateForm">
            @csrf
            @method('PUT')
            <input type="hidden" id="perawatan_uuid" name="uuid">
            ...
            <div class="space-y-4">
              <div>
                ...
                <input type="text" name="kode_perawatan" id="kode_perawatan" autocomplete="off"
                  class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2">
                <div class="text-red-500 text-sm mt-1" id="error-kode_perawatan"></div>
              </div>
              ...
              <div>
                ...
                <input type="text" name="perawatan" id="perawatan" autocomplete="off"
                  class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2">
                <div class="text-red-500 text-sm mt-1" id="error-perawatan"></div>
              </div>
            </div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
      ...
      function fetchItemData(uuid) {
        fetch(`/jenis-barang/edit/${uuid}`)
          ...
          .then(data => {
            document.getElementById('perawatan_uuid').value = uuid;
            form['kode_perawatan'].value = data.kode_perawatan;
            form['perawatan'].value = data.perawatan;
          });
      }
      ...
      form.addEventListener('submit', function(event) {
        ...
        const uuid = form['perawatan_uuid'].value;
